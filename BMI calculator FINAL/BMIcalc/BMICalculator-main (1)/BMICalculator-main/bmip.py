...
def BMI():
    ...
    if bmi <= 18.5:
        ...
        label3.config(
            text=mycursor_fetch_any("bmi_vals", "bmi_getter", "name = 'Underweight'")[0][0]
        )
        ...
        label3.config(
            text=mycursor_fetch_any("bmi_vals", "bmi_getter", "name = 'Overweight'")[0][0]
        )
...
def add_data():
    if float(Height.get()) == 0.00 or float(Weight.get()) == 0.00:
        ...
        tup = (BMIval.get(), Height.get(), Weight.get())
        print(tup)
        bmi_update(tup)
